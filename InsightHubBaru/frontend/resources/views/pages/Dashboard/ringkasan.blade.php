<div id="tab-ringkasan" class="dashboard-container active">
    ...
    <div class="page-header" style="margin-bottom: 2rem;">
        ...
        <div style="display: flex; flex-direction: row; align-items: center; justify-content: flex-start; flex-wrap: nowrap; gap: 12px; margin-top: 0.5rem;">
            <form id="filterRingkasanForm" method="GET" action="{{ route('dashboard') }}" style="display: contents;">
                ...
                <select name="ringkasan_periode" id="ringkasanPeriode" onchange="toggleRingkasanFilters(); this.form.submit()" style="padding: 0.5rem 1rem; border-radius: 2rem; border: 1px solid #e2e8f0; background: #fff; color: #0f172a; font-weight: 500; font-size: 0.85rem; cursor: pointer; outline: none; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
                    <option value="semua" {{ $ringkasanPeriode === 'semua' ? 'selected' : '' }}>Semua Waktu</option>
                    <option value="bulanan" {{ $ringkasanPeriode === 'bulanan' ? 'selected' : '' }}>Bulanan</option>
                    <option value="tahunan" {{ $ringkasanPeriode === 'tahunan' ? 'selected' : '' }}>Tahunan</option>
                </select>
                ...
                <select name="ringkasan_bulan" id="ringkasanBulan" onchange="this.form.submit()" style="padding: 0.5rem 1rem; border-radius: 2rem; border: 1px solid #e2e8f0; background: #fff; color: #0f172a; font-weight: 500; font-size: 0.85rem; cursor: pointer; outline: none; box-shadow: 0 1px 2px rgba(0,0,0,0.05); display: {{ $ringkasanPeriode === 'bulanan' ? 'inline-block' : 'none' }};">
                    @foreach($bulanList as $num => $nama)
                        <option value="{{ $num }}" {{ $ringkasanBulan == $num ? 'selected' : '' }}>{{ $nama }}</option>
                    @endforeach
                </select>

                <select name="ringkasan_tahun" id="ringkasanTahun" onchange="this.form.submit()" style="padding: 0.5rem 1rem; border-radius: 2rem; border: 1px solid #e2e8f0; background: #fff; color: #0f172a; font-weight: 500; font-size: 0.85rem; cursor: pointer; outline: none; box-shadow: 0 1px 2px rgba(0,0,0,0.05); display: {{ in_array($ringkasanPeriode, ['bulanan', 'tahunan']) ? 'inline-block' : 'none' }};">
                    @foreach($availableYears as $thn)
                        <option value="{{ $thn }}" {{ $ringkasanTahun == $thn ? 'selected' : '' }}>{{ $thn }}</option>
                    @endforeach
                </select>
                ...
                <select id="filterPlatform" name="ringkasan_platform" onchange="this.form.submit()" style="padding: 0.5rem 1rem; border-radius: 2rem; border: 1px solid #e2e8f0; background: #fff; color: #0f172a; font-weight: 500; font-size: 0.85rem; cursor: pointer; outline: none; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
                    <option value="Semua Platform" {{ $ringkasanPlatform == 'Semua Platform' ? 'selected' : '' }}>Semua Platform</option>
                    <option value="Instagram" {{ $ringkasanPlatform == 'Instagram' ? 'selected' : '' }}>Instagram</option>
                    <option value="TikTok" {{ $ringkasanPlatform == 'TikTok' ? 'selected' : '' }}>TikTok</option>
                    <option value="Facebook" {{ $ringkasanPlatform == 'Facebook' ? 'selected' : '' }}>Facebook</option>
                    <option value="YouTube" {{ $ringkasanPlatform == 'YouTube' ? 'selected' : '' }}>YouTube</option>
                </select>
            </form>
    ...
    <div class="rk-kpi-grid">
        ...
        <div class="rk-kpi-card">
            ...
            <div class="rk-kpi-value" id="kpi-total-konten">{{ number_format($totalKonten) }}</div>
        </div>
        ...
        <div class="rk-kpi-card">
            ...
            <div class="rk-kpi-value" id="kpi-total-engagement">{{ number_format($totalEngagement) }}</div>
        </div>

        {{-- 4. Total Followers Akun (nilai statis per platform, diupdate via JS) --}}
        <div class="rk-kpi-card">
            <div class="rk-kpi-top">
        ...
        <div class="rk-kpi-card">
            ...
            <div class="rk-kpi-value" id="kpi-total-reach">{{ number_format($totalReach) }}</div>
        </div>
        ...
        <div class="rk-kpi-card">
            ...
            <div class="rk-kpi-value" id="kpi-engagement-rate">{{ number_format($engagementRate, 2) }}%</div>
        </div>

        {{-- 7. Pertumbuhan Followers --}}
        <div class="rk-kpi-card">
            <div class="rk-kpi-top">
            ...
            <div class="rk-kpi-value" id="kpi-pertumbuhan-followers">+{{ number_format((int)($totalFollowers * 0.012)) }}</div>
        </div>
    ...
    <div class="card" style="margin-bottom: 2rem; border: none; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); border-radius: 1rem;">
        ...
        <div style="border-top: 1px solid #e2e8f0; padding-top: 1.5rem; margin-top: 1.5rem;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
                ...
                @foreach(['Instagram', 'TikTok', 'Facebook', 'YouTube'] as $name)
                @if(isset($platformsData[$name]))
                @php
                ...
                @endphp
                <div class="platform-card" data-platform="{{ $name }}" style="border: 1px solid #e2e8f0; border-radius: 0.75rem; padding: 1.25rem; background: #fafafa; display: {{ ($ringkasanPlatform == 'Semua Platform' || $ringkasanPlatform == $name) ? 'block' : 'none' }};">
                    <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1rem;">
                        <div style="width: 36px; height: 36px; border-radius: 8px; display: flex; align-items: center; justify-content: center; background: {{ $pData['soft'] }}; color: {{ $pData['color'] }};">
                            @if(strtolower($name) == 'instagram')
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line></svg>
                            @elseif(strtolower($name) == 'facebook')
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"></path></svg>
                            @elseif(strtolower($name) == 'tiktok')
                            ...
                            @elseif(strtolower($name) == 'youtube')
                    ...
                    <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem; font-size: 0.85rem;">
                        ...
                        <span style="font-weight: 600; color: #0f172a;">{{ number_format($pAkunFollowers) }}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem; font-size: 0.85rem;">
                        <span style="color: #64748b;">Reach:</span>
                        <span style="font-weight: 600; color: #0f172a;">{{ number_format($pTotalReach) }}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem; font-size: 0.85rem;">
                        <span style="color: #64748b;">Engagement:</span>
                        <span style="font-weight: 600; color: #0f172a;">{{ number_format($pTotalEng) }}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; font-size: 0.85rem;">
                        <span style="color: #64748b;">Total Konten:</span>
                        <span style="font-weight: 600; color: #0f172a;">{{ number_format($pTotalKonten) }}</span>
                    </div>
                </div>
                @endif
                @endforeach
    ...
    <div style="display: grid; grid-template-columns: 1fr; gap: 2rem; align-items: start;">
        ...
        <div class="card" style="border: none; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); border-radius: 1rem;">
            ...
            <div class="content-list" style="display: flex; flex-direction: column; gap: 1rem; margin-top: 1rem;">
                @forelse($metricsRingkasan->take(5) as $metric)
                <div class="content-item" style="display: flex; gap: 1rem; padding: 1rem; border: 1px solid #f1f5f9; border-radius: 0.75rem; align-items: center; background: #fff;">
                    ...
                    @endphp
                    <div style="width: 60px; height: 60px; background: {{ $platBg }}; color: {{ $platColor }}; border-radius: 0.5rem; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                        ...
                        @elseif($platLower == 'instagram')
                            <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line></svg>
                        @elseif($platLower == 'tiktok')
                    ...
                    <div style="flex: 1; min-width: 0;">
                        ...
                        <div style="display: flex; gap: 0.75rem; font-size: 0.8rem; color: #64748b; margin-bottom: 0.5rem; align-items: center;">
                            <span style="background: #e2e8f0; padding: 0.15rem 0.5rem; border-radius: 1rem; color: #475569; font-weight: 500;">{{ $metric->kategori ?: 'Tanpa Kategori' }}</span>
                            <span><i class="ph-bold ph-calendar-blank"></i> {{ $metric->tgl_upload ? \Carbon\Carbon::parse($metric->tgl_upload)->format('d M Y') : '-' }}</span>
                        </div>
                        ...
                        <div style="display: flex; gap: 1.25rem; font-size: 0.85rem; color: #475569; font-weight: 500; align-items: center;">
                            <div style="display: flex; align-items: center; gap: 0.35rem;" title="Views">
                                ...
                                <span>{{ number_format($metric->reach) }}</span>
                            </div>
                            <div style="display: flex; align-items: center; gap: 0.35rem;" title="Likes">
                                ...
                                <span>{{ number_format($metric->likes) }}</span>
                            </div>
                            <div style="display: flex; align-items: center; gap: 0.35rem;" title="Comments">
                                ...
                                <span>{{ number_format($metric->comments) }}</span>
                            </div>
                            <div style="display: flex; align-items: center; gap: 0.35rem;" title="Reposts / Shares">
                                ...
                                <span>{{ number_format($metric->shares) }}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div style="display: flex; flex-direction: column; gap: 1.25rem;">
                ...
                <div>
                    ...
                    <div style="font-size: 1.05rem; font-weight: 600;">
                        @if($highestEngPlatformName != '-')
                            <span style="color: #60a5fa;">{{ $highestEngPlatformName }}</span> memimpin dengan rasio tertinggi.
                        @else
                ...
                <div>
                    ...
                    <div style="font-size: 1.05rem; font-weight: 600; line-height: 1.4;">
                        @if($topContent)
                            "{{ \Illuminate\Support\Str::limit($topContent->judul_konten ?: 'Konten ' . ucfirst($topContent->platform), 30) }}" 
                            <span style="color: #34d399; font-size: 0.95rem;">({{ number_format($topContent->reach) }} views)</span>
                        @else
                ...
                <div>
                    ...
                    <div style="font-size: 1.05rem; font-weight: 600;">
                        @if($highestGrowthPlatformName != '-')
                            Fokus pada <span style="color: #c084fc;">{{ $highestGrowthPlatformName }}</span> memberikan pertumbuhan audiens terbanyak.
                        @else
                            Data belum mencukupi.
                        @endif
                    </div>
...
<div id="exportRingkasanModal" class="lap-modal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 9999; justify-content: center; align-items: center;">
    <div class="lap-modal-content" style="background: #fff; width: 400px; border-radius: 8px; padding: 20px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
        ...
        <form action="{{ route('export-requests.store') }}" method="POST">
            @csrf
            ...
            {{-- Base64 chart images — diisi oleh JS sebelum modal dibuka --}}
...
const platformDataRaw = @json($jsPlatformData);
...
function updateDashboardFilter() {
    ...
    const badgeDeltas = @json($badgeDeltas);
    if (badgeDeltas) {
