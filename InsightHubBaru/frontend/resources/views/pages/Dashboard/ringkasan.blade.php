<div id="tab-ringkasan" class="dashboard-container active">
    ...
    <div class="rk-kpi-grid">
        ...
        <div class="rk-kpi-card">
            ...
            <div class="rk-kpi-value" id="kpi-total-followers">{{ number_format($totalFollowers) }}</div>
        </div>
    ...
    <div class="card" style="margin-bottom: 2rem; border: none; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); border-radius: 1rem;">
        ...
        <div style="border-top: 1px solid #e2e8f0; padding-top: 1.5rem; margin-top: 1.5rem;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
                ...
                @endphp
                <div class="platform-card" data-platform="{{ $name }}" style="border: 1px solid #e2e8f0; border-radius: 0.75rem; padding: 1.25rem; background: #fafafa;">
                    <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 1rem;">
                        <i class="ph-fill {{ $pData['icon'] }}" style="color: {{ $pData['color'] }}; font-size: 1.5rem;"></i>
                        <span style="font-weight: 700; color: #0f172a;">{{ $name }}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem; font-size: 0.85rem;">
                        <span style="color: #64748b;">Followers:</span>
                        <span style="font-weight: 600; color: #0f172a;">{{ number_format($pTotalSubs) }}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem; font-size: 0.85rem;">
    ...
    <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 2rem; align-items: start;">
        ...
        <div class="card" style="border: none; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); border-radius: 1rem;">
            ...
            <div class="content-list" style="display: flex; flex-direction: column; gap: 1rem; margin-top: 1rem;">
                @forelse($metrics->take(5) as $metric)
                <div class="content-item" style="display: flex; gap: 1rem; padding: 1rem; border: 1px solid #f1f5f9; border-radius: 0.75rem; align-items: center; background: #fff;">
                    ...
                    <div style="width: 60px; height: 60px; background: #f8fafc; border-radius: 0.5rem; display: flex; align-items: center; justify-content: center; font-size: 1.75rem; flex-shrink: 0;">
                        @if(strtolower($metric->platform) == 'facebook') <i class="ph-fill ph-facebook-logo" style="color: #1877f2;"></i>
                        @elseif(strtolower($metric->platform) == 'instagram') <i class="ph-fill ph-instagram-logo" style="color: #e1306c;"></i>
                        @elseif(strtolower($metric->platform) == 'tiktok') <i class="ph-fill ph-tiktok-logo" style="color: #000000;"></i>
                        @elseif(strtolower($metric->platform) == 'youtube') <i class="ph-fill ph-youtube-logo" style="color: #ff0000;"></i>
                        @else <i class="ph-fill ph-file-video"></i>
                        @endif
                    </div>
                    ...
                    <div style="flex: 1; min-width: 0;">
                        ...
                        <div style="display: flex; gap: 1rem; font-size: 0.85rem; color: #334155;">
                            <span title="Reach"><i class="ph-bold ph-eye" style="color: #10b981;"></i> {{ number_format($metric->reach) }}</span>
                            <span title="Like"><i class="ph-bold ph-heart" style="color: #e1306c;"></i> {{ number_format($metric->likes) }}</span>
                            <span title="Comment"><i class="ph-bold ph-chat-circle" style="color: #3b82f6;"></i> {{ number_format($metric->comments) }}</span>
                            <span title="Share"><i class="ph-bold ph-share-network" style="color: #8b5cf6;"></i> {{ number_format($metric->shares) }}</span>
                        </div>
                    </div>
                </div>
                @empty
...
const defaultChartData = {
    ...
    reach: @json($chartReach),
    engagement: @json($chartEngagement),
    likes: @json($chartLikes),
    comments: @json($chartComments),
    shares: @json($chartShares),
    totalKonten: @json($chartTotalKonten)
};
