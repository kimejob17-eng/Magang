    ...
    <form method="GET" action="{{ route('dashboard') }}" id="laporanFilterForm">
        ...
        <div class="lap-filter-card">
            ...
            <div class="lap-filter-row">
                ...
                {{-- Input: Rentang Tanggal (shown/hidden by JS via id) --}}
                ...
                {{-- Input: Bulan & Tahun (shown/hidden by JS via id) --}}
                ...
                {{-- Input: Tahun saja (shown/hidden by JS via id) --}}
                ...
                <div class="lap-filter-actions">
                    ...
                    <a href="{{ route('dashboard') }}?tab=laporan" class="lap-btn-reset">
    ...
    <div class="lap-table-card">
        ...
        <div class="lap-sort-bar">
            ...
            <div class="lap-sort-identity">
                ...
                <div class="lap-sort-identity-text">
                    ...
                    <span class="lap-sort-identity-value">{{ $activeLabel }}</span>
                </div>
            </div>

            {{-- Center: grouped select + direction toggle --}}
            <div class="lap-sort-group">
                ...
                >
                    ...
                    <option
                        value="{{ $buildSortUrl($col) }}"
                        {{ $lapFilters['sort'] === $col ? 'selected' : '' }}
                    >{{ $label }}</option>
                    @endforeach
                </select>
                <a
                    href="{{ $currentSortUrl }}"
                    id="lapSortDirBtn"
                    class="lap-sort-dir-btn {{ $isAsc ? 'asc' : 'desc' }}"
                    title="{{ $isAsc ? 'Klik untuk urutan Turun (Z→A)' : 'Klik untuk urutan Naik (A→Z)' }}"
                >
                    @if($isAsc)
        ...
        <div class="lap-list">
            ...
            <div class="lap-row-card">
                ...
                {{-- Meta row: platform badge + date + jenis + kategori --}}
                <div class="lap-row-meta">
                    <div class="lap-row-meta-left">
                        <div class="lap-item-platform-icon" style="background: {{ $pSoft }}; color: {{ $pColor }}; border-radius: 6px; display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px;">
                            @if($platKey == 'instagram')
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line></svg>
                            @elseif($platKey == 'facebook')
                            ...
                            @elseif(str_contains($platKey, 'youtube') || str_contains($platKey, 'yt'))
                        ...
                        <span class="lap-plat-name">{{ ucfirst($row->platform) }}</span>
                        ...
                        <span class="lap-row-date">
                            ...
                            {{ $row->tgl_upload ? \Carbon\Carbon::parse($row->tgl_upload)->format('d M Y') : '-' }}
                        </span>
                        @if($row->jenis)
                        <span class="lap-dot"></span>
                        <span class="lap-pill" style="display: inline-flex; align-items: center; gap: 0.25rem;">
                            ...
                            @elseif(str_contains($n, 'video') || str_contains($n, 'stream'))
                            ...
                            @elseif(str_contains($n, 'post') || str_contains($n, 'image') || str_contains($n, 'feed') || str_contains($n, 'photo'))
                    ...
                    @if($row->kategori)
                    <span class="lap-cat-pill">{{ $row->kategori }}</span>
                    @endif
                </div>

                {{-- Content title --}}
                <div class="lap-row-title">{{ $row->judul_konten ?: '(Tanpa Judul)' }}</div>

                {{-- Metric grid: 6 cols desktop, 3 tablet, 2 mobile --}}
                ...
                <div class="lap-link-row">
                    @if($row->tautan)
                    <a href="{{ $row->tautan }}" target="_blank" class="lap-link-btn">
        ...
        <div class="lap-pagination">
            <div>
                Menampilkan <b>{{ $metricsLaporan->firstItem() ?? 0 }}</b>
                sampai <b>{{ $metricsLaporan->lastItem() ?? 0 }}</b>
                dari <b>{{ $metricsLaporan->total() }}</b> data
            </div>
            <div class="lap-page-nav">
                ...
                @else
                    <a href="{{ $metricsLaporan->previousPageUrl() }}" class="lap-page-btn"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg> Prev</a>
                @endif

                <span class="lap-page-current">
                    {{ $metricsLaporan->currentPage() }} / {{ $metricsLaporan->lastPage() ?: 1 }}
                </span>

                @if($metricsLaporan->hasMorePages())
                    <a href="{{ $metricsLaporan->nextPageUrl() }}" class="lap-page-btn">Next <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg></a>
                @else
...
{{-- ============================================================
     MODAL PERMINTAAN EXPORT (USER ONLY)
============================================================ --}}
@if(auth()->check() && auth()->user()->role === 'user')
<div id="exportModal" class="lap-modal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 9999; justify-content: center; align-items: center;">
    <div class="lap-modal-content" style="background: #fff; width: 400px; border-radius: 8px; padding: 20px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
        ...
        <form action="{{ route('export-requests.store') }}" method="POST">
            ...
            @foreach(request()->except(['_token', 'type', 'reason', 'details']) as $key => $value)
                @if(is_array($value))
                    @foreach($value as $arrKey => $arrVal)
                        <input type="hidden" name="filters[{{ $key }}][{{ $arrKey }}]" value="{{ $arrVal }}">
                    @endforeach
                @else
