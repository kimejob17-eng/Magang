{{-- ---- YOUTUBE LIVE FORM ---- --}}
<div id="yt-tab-live" class="yt-tab-content">
    <div class="platform-form-card">
        <div class="platform-form-header" style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 1rem;">
            <div style="display: flex; gap: 1rem; align-items: center;">
                ...
                <div>
                    <h2 style="font-size: 1.1rem; font-weight: 700; color: #0f172a; margin-bottom: 0.25rem;">YouTube Live Data</h2>
                    <p style="font-size: 0.8rem; color: #64748b; margin: 0;">Manual data entry for YouTube Live Streams.</p>
                </div>
            </div>
            <button type="button" class="btn" onclick="openImportModal('yt-live')" style="cursor: pointer; position: relative; z-index: 10; background: #10b981; color: white; border: none; font-weight: 500; font-size: 0.85rem; padding: 0.5rem 1rem; border-radius: 2rem; display: flex; align-items: center; gap: 6px; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
        ...
        <form id="form-yt-live" action="{{ route('dashboard.metrics.store') }}" method="POST">
            @csrf
            <div id="method-container-yt-live"></div>
            <input type="hidden" name="platform" value="yt-live">
            <input type="hidden" name="metric_id" id="metric-id-yt-live" value="">
            <input type="hidden" name="content_type" value="Live Stream">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="title" class="form-control" placeholder="Contoh: Live Q&A Session" required>
                </div>
            </div>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    <label>Penonton Puncak</label>
                    <div style="position: relative;">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="position: absolute; left: 1rem; top: 50%; transform: translateY(-50%); color: #64748b;"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
                        <input type="text" name="peak_viewers" class="form-control" style="padding-left: 2.5rem;" placeholder="0">
                    </div>
                </div>
            </div>
            ...
            <div class="form-row" style="margin-top: 1rem;">
                <div class="form-group" style="margin-bottom: 0;">
                    <button type="button" id="btn-cancel-live" class="btn" style="display:none; width: 100%; justify-content: center; background: #f8fafc; color: #64748b;" onclick="resetFormYTLive()">Batal Edit</button>
                    <button type="reset" id="btn-reset-live" class="btn" style="width: 100%; justify-content: center; background: #f8fafc; color: #64748b;">Reset Form</button>
                </div>
                <div class="form-group" style="margin-bottom: 0;">
                    <button type="submit" id="btn-submit-yt-live" class="btn btn-primary" style="width: 100%; justify-content: center; background: #ff0000; border-color: #ff0000;"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 4px;"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"></path><polyline points="17 21 17 13 7 13 7 21"></polyline><polyline points="7 3 7 8 15 8"></polyline></svg> Simpan Data</button>
                </div>
            </div>
        </form>
    </div>
    ...
    <div class="card" style="margin-top: 2rem;">
        <div class="card-title" style="margin-bottom: 1rem;">Riwayat Data YouTube Live</div>
            ...
            <table class="data-table table-hover">
                <thead>
                    <tr>
                        ...
                        <th width="10%">Penonton Puncak</th>
                        <th width="16%" style="text-align: center;">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($metrics->filter(fn($m) => strtolower($m->platform) === 'youtube' && $m->jenis === 'Live Stream') as $metric)
                    ...
                    <tr>
                        ...
                        <td>{{ number_format($metric->penonton_puncak ?? 0) }}</td>
                        <td>
                            <div style="display:flex; gap:12px; align-items:center; justify-content: center;">
                                ...
                                <a href="javascript:void(0)" onclick="editLive('{{ $id_asli }}', {{ json_encode($metric) }})" title="Edit" style="color: #f59e0b; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.1)'" onmouseout="this.style.transform='scale(1)'">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 1 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                                </a>
                                <form action="{{ route('dashboard.metrics.destroy', ['yt-live', $id_asli]) }}" method="POST" style="margin:0;">
                                    @csrf @method('DELETE')
...
<script>
function editLive(id_konten, data) {
    const form = document.getElementById('form-yt-live');
    form.action = "{{ url('dashboard/metrics') }}/yt-live/" + id_konten;
    document.getElementById('method-container-yt-live').innerHTML = '<input type="hidden" name="_method" value="PUT">';
    document.getElementById('metric-id-yt-live').value = id_konten;
    ...
    form.querySelector('[name="peak_viewers"]').value = data.penonton_puncak || 0;
...
function resetFormYTLive() {
    const form = document.getElementById('form-yt-live');
    form.reset();
    form.action = "{{ route('dashboard.metrics.store') }}";
    document.getElementById('method-container-yt-live').innerHTML = '';
    document.getElementById('metric-id-yt-live').value = '';
    
    document.getElementById('btn-submit-yt-live').innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px;"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"></path><polyline points="17 21 17 13 7 13 7 21"></polyline><polyline points="7 3 7 8 15 8"></polyline></svg> Simpan Data';
    document.getElementById('btn-reset-live').style.display = 'flex';
    document.getElementById('btn-cancel-live').style.display = 'none';
    form.classList.remove('form-edit-highlight');
}
</script>