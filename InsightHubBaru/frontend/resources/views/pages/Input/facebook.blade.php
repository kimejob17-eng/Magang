    ...
    <div class="platform-form-card">
        <div class="platform-form-header" style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 1rem;">
            <div style="display: flex; gap: 1rem; align-items: center;">
                <div class="platform-form-icon" style="background: #e3edfd; color: #1877f2; display: flex; align-items: center; justify-content: center;">
                ...
                <div>
                    ...
                    <p style="font-size: 0.8rem; color: #64748b; margin: 0;">Manual data entry for Facebook posts and campaigns.</p>
                </div>
            </div>
            <button type="button" class="btn" onclick="openImportModal('facebook')" style="cursor: pointer; position: relative; z-index: 10; background: #10b981; color: white; border: none; font-weight: 500; font-size: 0.85rem; padding: 0.5rem 1rem; border-radius: 2rem; display: flex; align-items: center; gap: 6px; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
        ...
        <form id="form-facebook" action="{{ route('dashboard.metrics.store') }}" method="POST">
            @csrf
            <div id="method-container-facebook"></div>
            <input type="hidden" name="platform" value="facebook">
            <input type="hidden" name="metric_id" id="metric-id-facebook" value="">
            <div class="form-row">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="url" name="url" class="form-control" placeholder="https://facebook.com/...">
                </div>
            </div>
            ...
            <div class="form-row" style="margin-top: 1rem;">
                <div class="form-group" style="margin-bottom: 0;">
                    <button type="button" id="btn-cancel-facebook" class="btn" style="display:none; width: 100%; justify-content: center; background: #f8fafc; color: #64748b;" onclick="resetFormFacebook()">Batal Edit</button>
                    <button type="reset" id="btn-reset-facebook" class="btn" style="width: 100%; justify-content: center; background: #f8fafc; color: #64748b;">Reset Form</button>
                </div>
                <div class="form-group" style="margin-bottom: 0;">
                    <button type="submit" id="btn-submit-facebook" class="btn btn-primary" style="width: 100%; justify-content: center; background: #003EA8; border-color: #003EA8;"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 4px;"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"></path><polyline points="17 21 17 13 7 13 7 21"></polyline><polyline points="7 3 7 8 15 8"></polyline></svg> Save Data</button>
                </div>
            </div>
        </form>
    </div>
    ...
    <div class="card" style="margin-top: 2rem;">
        ...
        <div style="overflow-x: auto;">
            <table class="data-table table-hover">
                ...
                <tbody>
                    @forelse($metrics->filter(fn($m) => strtolower($m->platform) === 'facebook') as $metric)
                    @php $id_asli = explode('-', $metric->id_konten)[1] ?? $metric->id_konten; @endphp
                    <tr>
                        <td>{{ \Carbon\Carbon::parse($metric->tgl_upload)->format('d M, Y') }}</td>
                        <td style="font-weight: 500; max-width: 250px;">
                            <div class="topic-text" title="{{ $metric->judul_konten }}">{{ $metric->judul_konten }}</div>
                        </td>
                        <td><span class="badge badge-stable">{{ $metric->jenis }}</span></td>
                        <td>{{ number_format($metric->reach) }}</td>
                        <td>{{ number_format($metric->saves ?? 0) }}</td>
                        <td>
                            <div style="display:flex; gap:12px; align-items:center; justify-content: center;">
                                <a href="javascript:void(0)" onclick='openDetailModal(@json($metric))' title="Detail" style="color: var(--primary); transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.1)'" onmouseout="this.style.transform='scale(1)'">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
                                </a>
                                <a href="javascript:void(0)" onclick="editFacebook('{{ $id_asli }}', {{ json_encode($metric) }})" title="Edit" style="color: #f59e0b; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.1)'" onmouseout="this.style.transform='scale(1)'">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 1 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                                </a>
                                <form action="{{ route('dashboard.metrics.destroy', ['facebook', $id_asli]) }}" method="POST" style="margin:0;">
                                    @csrf @method('DELETE')
...
<script>
function editFacebook(id_konten, data) {
    const form = document.getElementById('form-facebook');
    form.action = "{{ url('dashboard/metrics') }}/facebook/" + id_konten;
    document.getElementById('method-container-facebook').innerHTML = '<input type="hidden" name="_method" value="PUT">';
    document.getElementById('metric-id-facebook').value = id_konten;
    ...
    form.querySelector('[name="content_type"]').value = data.jenis || 'Image Post';
    ...
    document.getElementById('btn-submit-facebook').innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px;"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 1 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg> Update Data';
    document.getElementById('btn-reset-facebook').style.display = 'none';
    document.getElementById('btn-cancel-facebook').style.display = 'flex';
    
    // Animate form to show it's in edit mode
...
function resetFormFacebook() {
    const form = document.getElementById('form-facebook');
    form.reset();
    form.action = "{{ route('dashboard.metrics.store') }}";
    document.getElementById('method-container-facebook').innerHTML = '';
    document.getElementById('metric-id-facebook').value = '';
    
    document.getElementById('btn-submit-facebook').innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px;"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"></path><polyline points="17 21 17 13 7 13 7 21"></polyline><polyline points="7 3 7 8 15 8"></polyline></svg> Save Data';
    document.getElementById('btn-reset-facebook').style.display = 'flex';
    document.getElementById('btn-cancel-facebook').style.display = 'none';
    form.classList.remove('form-edit-highlight');
}
</script>