...
<div class="mk-container">
    ...
    <div class="mk-card" style="margin-bottom: 24px;">
        <div class="mk-card-body" style="padding: 20px;">
            <form id="user-select-form" action="{{ route('role-mapping.index') }}" method="GET" style="display: flex; flex-direction: row; gap: 16px; align-items: flex-end; width: 100%; flex-wrap: wrap;">
                    ...
                    <select name="user_id" id="user_id_select" class="mk-input" onchange="document.getElementById('user-select-form').submit()" style="cursor: pointer; width: 100%; appearance: none; background-image: url('data:image/svg+xml;utf8,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%2216%22 height=%2216%22 fill=%22%23475569%22 viewBox=%220 0 256 256%22><path d=%22M213.66,101.66l-80,80a8,8,0,0,1-11.31,0l-80-80a8,8,0,0,1,11.31-11.31L128,164.69l74.34-74.34a8,8,0,0,1,11.31,11.31Z%22></path></svg>'); background-repeat: no-repeat; background-position: right 14px center; padding-right: 40px;">
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ $selectedUser->id === $user->id ? 'selected' : '' }}>
                                {{ $user->name }} ({{ $user->username }}) — Role Utama: {{ ucfirst($user->role) }}
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            @if($isCustomRole)
                <form id="reset-form" action="{{ route('role-mapping.reset') }}" method="POST" style="display: none;">
                    @csrf
    ...
    <form action="{{ route('role-mapping.update') }}" method="POST">
        @csrf
        <input type="hidden" name="user_id" value="{{ $selectedUser->id }}">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 12px;">
                    <div>
                        <div class="mk-card-title" style="display: flex; align-items: center; flex-wrap: wrap; gap: 8px;">
                            <span>Mengonfigurasi Akses: <strong>{{ $selectedUser->name }}</strong></span>
                            <span class="mk-badge {{ $selectedUser->role === 'super-admin' ? 'mk-badge-super' : ($selectedUser->role === 'admin' ? 'mk-badge-admin' : 'mk-badge-user') }}">
                        ...
                        <div style="font-size: 13px; color: #64748b; margin-top: 4px;">
                            Email: <strong>{{ $selectedUser->email }}</strong> | Username: <strong>{{ $selectedUser->username }}</strong>
                        </div>
                    </div>
                        ...
                        <thead>
                            <tr>
                                ...
                                @foreach($permissions as $permission)
                                    <th style="text-align: center; width: 15%;">{{ $permission->name }}</th>
                                @endforeach
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($menus as $menu)
                                ...
                                <tr class="menu-header-row">
                                    <td colspan="{{ 1 + count($permissions) }}">
                                        <div style="display: flex; align-items: center; justify-content: space-between;">
                                            <div>
                                                <input type="checkbox" 
                                                       class="menu-parent-checkbox" 
                                                       data-menu-id="{{ $menu->id }}" 
                                                       {{ $menuHasActivePermission ? 'checked' : '' }}
                                                ...
                                                <span style="vertical-align: middle; color: #1e293b; font-weight: 700;">{{ $menu->name }}</span>
                                            </div>
                                ...
                                @if($menu->details->isEmpty())
                                    <tr>
                                        <td colspan="{{ 1 + count($permissions) }}" style="text-align: center; color: #94a3b8; padding: 16px; font-style: italic;">
                                        ...
                                        <tr>
                                            ...
                                            @foreach($permissions as $permission)
                                                ...
                                                <td class="checkbox-cell">
                                                    <input type="checkbox" 
                                                           name="mapping[{{ $detail->id }}][{{ $permission->id }}]" 
                                                           value="1" 
                                                           class="checkbox-input child-of-{{ $menu->id }}" 
                                                           data-menu-id="{{ $menu->id }}"
                                                           {{ $checked ? 'checked' : '' }}>
                                                </td>
                                            @endforeach
